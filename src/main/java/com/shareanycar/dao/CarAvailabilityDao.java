...
public class CarAvailabilityDao  extends BasicDao<CarAvailability>{
	...
	public List<CarAvailability> findCarAvailabilityByCarId(Long carId){
		return (List<CarAvailability>) extDao.findListByParam("from CarAvailability where car_id = :car_id", "car_id", carId);
	}
	
	public CarAvailability findCarAvailabilityByParams(Long carId, LocalDate date) {
		try {
			return (CarAvailability) extDao.findOneByParams("from CarAvailability where car_id = :car_id and date = :date", new String[]{"car_id", "date"}, new Object[]{carId, date});
		} catch (Exception e) {
			return null;
		}
	}
	...
	public List<CarAvailability> findCarAvailablityByParams(Long carId, LocalDate dateFrom, LocalDate dateTo){
		try {
			...
			return new ArrayList<CarAvailability>();
		}
	}
}
